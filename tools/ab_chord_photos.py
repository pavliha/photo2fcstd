"""The loop guards - keep_simple and drop_stray_holes - on real photographs, with a build check.
...
def one(args):
    ...
    trace.KEEP_SIMPLE = ARMS[arm]
    trace.DROP_STRAY_HOLES = ARMS[arm]
    try:
...
def main(limit=160):
    ...
    for key in ("region_iou", "structure"):
        d = np.array([by["guards"][p][key] - by["before"][p][key] for p in keen])
        m, lo, hi = stats.mean_ci(d)
        print("\n  %-12s guards vs before %+.4f [%+.4f, %+.4f]" % (key, m, lo, hi))
