...
CEIL = json.load(open(os.path.join(ROOT, "data", "eps_ceiling_big.json")))
...
def one(part):
    ...
    try:
        ...
        raw = v["shape"]["raw"]
        return part, EM.per_candidate(v, raw, EM.element_counts(raw, v["length_px"])).tolist()
    except Exception:
        return part, None
...
def main():
    ...
    best = I.argmax(1)
    X = np.array([r for p in parts for r in feats[p]], float)
    y = np.array([int(i == best[j]) for j in range(len(parts)) for i in range(len(EM.CANDIDATES))])
    g = np.repeat(np.arange(len(parts)), len(EM.CANDIDATES))
    ...
    for tr, te in GroupKFold(n_splits=5).split(X, y, g):
        ...
        for j in np.unique(g[te]):
            pred[j] = int(np.argmax(prob[g[te] == j]))
    y_best = best
    ...
    print("  picks the best tolerance %.0f%% of the time (chance %.0f%%)"
          % (100 * np.mean(pred == y_best), 100 / len(EM.CANDIDATES)))
