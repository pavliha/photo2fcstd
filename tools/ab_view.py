"""A/B the learned view choice on parts the model was never trained on."""
import json, os, sys
from multiprocessing import Pool

import numpy as np

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, os.path.join(ROOT, "src"))
IDEAL = json.load(open(os.path.join(ROOT, "data", "printcad_ideal_sketches_all.json")))
TRAINED_ON = set(json.load(open(os.path.join(ROOT, "data", "view_ceiling.json"))))


def one(part):
    from photo2fcstd import analysis, bench, sketch_score as SS, spec as spec_mod
    try:
        views = [analysis.view(p) for p in bench.photos_of(part)[:3]]
        doc = spec_mod.assemble(views, name=part, log=lambda *a: None)
        s = SS.score_one(doc, IDEAL[part])
        return part, {"iou": s["region_iou"], "trivial": s["trivial"], "mode": doc["mode"],
                      "source": os.path.basename((doc.get("outline") or doc.get("revolve")
                                                  or {"source": "?"})["source"]),
                      "exact": s["counts_mine"] == s["counts_ideal"]}
    except Exception as e:
        return part, {"error": str(e)[:80]}


def main(limit=300):
    from photo2fcstd import bench, sketch_score as SS
    parts = [p for p in sorted(IDEAL) if SS.trustworthy(IDEAL[p]) and p not in TRAINED_ON]
    parts = bench.with_photos(parts)[0][:limit]
    with Pool(6) as pool:
        out = dict(pool.map(one, parts))
    arm = "view_model" if os.environ.get("P2F_VIEW_MODEL") == "1" else "shipped"
    json.dump(out, open(os.path.join(ROOT, "data", "ab_view_%s.json" % arm), "w"))
    ok = [v for v in out.values() if "iou" in v]
    print("%s: %d of %d parts produced a sketch" % (arm, len(ok), len(parts)))


if __name__ == "__main__":
    main()
