"""A7 attempt: chains of same-turning line pieces refit as one arc.

Screen on perfect rasterised faces first (the mechanism), then real photographs with builds
(the decision). The flag is set for both arms on every job - a pool worker is reused, so patching
one arm leaves it patched for every later job.
"""
...
def build(specs, tag):
    run = os.path.join(ROOT, "runs", "chain_" + tag, "out")
    os.makedirs(run, exist_ok=True)
    listing = os.path.join(ROOT, "runs", "chain_" + tag, "list.txt")
    rows = []
...
def main(limit=420, do_photos=1):
    ...
    for a in pby:
        for v in pby[a].values():
            v.pop("spec", None)
    json.dump(pby, open(os.path.join(ROOT, "data", "ab_chain.json"), "w"))
