...
def build(specs, tag):
    run = os.path.join(ROOT, "runs", "bspl_" + tag, "out")
    os.makedirs(run, exist_ok=True)
    listing = os.path.join(ROOT, "runs", "bspl_" + tag, "list.txt")
    rows = []
...
def main(limit=420, do_photos=1):
    ...
    for a in pby:
        for v in pby[a].values():
            v.pop("spec", None)
    json.dump(pby, open(os.path.join(ROOT, "data", "ab_bspline.json"), "w"))
