"""A/B for the bsplinecurve primitive: chains that are provably curved but not circular.

The flag is set for both arms on every job - a pool worker is reused.
"""
...
def build(specs, tag):
    run = os.path.join(ROOT, "runs", "tang_" + tag, "out")
    os.makedirs(run, exist_ok=True)
    listing = os.path.join(ROOT, "runs", "tang_" + tag, "list.txt")
    rows = []
...
def main(limit=420, do_photos=1):
    ...
    bspl = [p for p in keen if any(e.get("type") == "bsplinecurve"
            for l in (IDEAL[p].get("loops") or []) for e in l)]
    report(by, keen, "PERFECT INPUT, all")
    report(by, bspl, "PERFECT INPUT, parts whose ideal holds a bspline")
    if not do_photos:
        return
    ...
    bspl = [p for p in keen if any(e.get("type") == "bsplinecurve"
            for l in (IDEAL[p].get("loops") or []) for e in l)]
    report(pby, keen, "PHOTOGRAPHS, all")
    report(pby, bspl, "PHOTOGRAPHS, parts whose ideal holds a bspline")
    ...
    for a in pby:
        for v in pby[a].values():
            v.pop("spec", None)
    json.dump(pby, open(os.path.join(ROOT, "data", "ab_tangent.json"), "w"))
