"""How often does the learned mode model route a part to a mode that draws nothing?"""
import json, os, sys
...
def main(limit=260):
    ...
    json.dump(out, open(os.path.join(ROOT, "data", "ab_stations_%s.json" % tag), "w"))
    ok = [v for v in out.values() if "iou" in v]
    drew = sum(1 for v in ok if v["drew"])
    print("mode model %s: %d parts, %d drew a sketch (%.0f%%)" % (tag, len(ok), drew, 100 * drew / max(len(ok), 1)))
